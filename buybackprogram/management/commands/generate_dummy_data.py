...
from buybackprogram.models import Contract, Tracking, Program, EveType, TrackingItem

User = get_user_model()

class Command(BaseCommand):
    help = 'Generates dummy data for contracts, tracking objects, and tracking items'

    def add_arguments(self, parser):
        parser.add_argument('-n', '--number', type=int, default=1000, help='Number of contracts and tracking objects to create')

    def handle(self, *args, **options):
        # Prompt the user for confirmation
        self.stdout.write(self.style.WARNING('This command will delete all existing contracts and tracking objects.'))
        ...
        self.stdout.write(self.style.WARNING('Deleting all existing contracts and tracking objects...'))
        Contract.objects.all().delete()
        Tracking.objects.all().delete()
        
        self.stdout.write(self.style.SUCCESS('All existing contracts and tracking objects have been deleted.'))
        number = options['number']
        
        self.stdout.write(self.style.SUCCESS(f'Starting generation of {number} dummy contracts, tracking objects, and tracking items'))

        

        for _ in range(number):
            user = User.objects.order_by('?').first()
            character = CharacterOwnership.objects.filter(user=user).order_by('?').first()
            ...
            date_expired = date_issued + timedelta(days=90)  # Contract expires 90 days after issue
            ...
            for _ in range(randint(1, 5)):  # Random number of items from 1 to 5
                eve_type = EveType.objects.order_by('?').first()  # Randomly select an item type
                if eve_type:
            ...
            self.stdout.write(self.style.SUCCESS(f'Generated contract {contract.contract_id}, tracking {tracking.id}, and tracking items'))

        self.stdout.write(self.style.SUCCESS(f'Finished generating {number} dummy contracts, tracking objects, and tracking items'))
